...
`response_foreign_id` | String | `[Deprecated]`. Will be renamed `foreign_id`. Internal Response ID given by the provider
`answers` | Hash | `[Deprecated]` Survey question and responses
`foreign_id` | String | `[Deprecated]`. Will be replaced by the current content of `response_foreign_id`. Internal Response ID generated by Engage Digital.
