...
`questions` | Array of Hashes | List of questions containing the displayed `text` of the question, its `foreign_id` (from Alchemer) and their `replies` in the form of an Array. Each one of the `replies` entry contains two keys: `text` and `value` of the customer reply.
